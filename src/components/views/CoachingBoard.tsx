...
// Coaching board + Log Coaching side panel (ported from app/coaching_view.jsx).
// Search by advocate, pull AI recommendations, log coaching + mark complete
// (green border), sort by completed/incomplete, filter by class/cohort/trainer.
// Priority (pink) coachings = release reviews auto-assigned to Shelby Gary.
// Exports: CoachingBoard, LogCoachingPanel, buildCoachingList.

import { useEffect, useState } from "react";
import {
  CATEGORIES, LEVELS, TRAINERS, advocateById, classesOfTrainer, cohortById,
  cohorts, extraSamples, baseCoachings, trainerById,
} from "@/lib/nest-data";
...
import { Dropdown, Segmented, Avatar, EmptyState } from "@/components/ui";
import { NIcon } from "@/components/icons";
import { SummaryTile } from "./Classes";
import type { Advocate, Category, Coaching, NestClass } from "@/lib/types";

type Sel = { key: string; note: string };

export interface EnrichedCoaching extends Coaching {
  status: "complete" | "incomplete";
  notes: string;
  completedAt: string | null;
  coachId: string | null;
  classroomTrainer: string | null;
  strengths: Sel[];
  opportunities: Sel[];
  advocate: Advocate;
  cohort?: NestClass | null;
}

const CANNED_LOG =
  "Reviewed recent calls together and modeled the target behavior in a live roleplay. Advocate engaged well and showed immediate improvement. Agreed on a follow-up practice set before the next assessment.";

export const buildCoachingList = (S: StoreState): EnrichedCoaching[] => {
  const all: any[] = [...baseCoachings, ...extraSamples, ...S.extraCoachings];
  // de-dupe by id (store entries win)
  const seen = new Set<string>();
  const out: any[] = [];
  all.forEach((r) => { if (!seen.has(r.id)) { seen.add(r.id); out.push(r); } });
  return out.map((rec) => {
    const a = advocateById(rec.advId);
    const c = a ? cohortById(a.cohort) : null;
  ...
  }).filter((r) => r.advocate) as EnrichedCoaching[];
};

interface CoachingBoardProps {
...
export const CoachingBoard = ({ onNav, onOpenPlan, onOpenAdvocate, search, onSearch }: CoachingBoardProps) => {
  const store = useStore();
  const S = store.get();
  const [q, setQ] = useState("");
  const [status, setStatus] = useState("all");
  const [filterBy, setFilterBy] = useState("all");   // all | class:<id> | trainer:<id> | level:<lvl>
  const [logFor, setLogFor] = useState<EnrichedCoaching | null>(null);

  let list = buildCoachingList(S);
  ...
  list.sort((a, b) => (Number(b.priority) - Number(a.priority)) || (Number(a.status === "complete") - Number(b.status === "complete")));

  const counts = buildCoachingList(S).reduce(
    (acc, r) => { acc[r.status]++; if (r.priority && r.status !== "complete") acc.priority++; return acc; },
    { complete: 0, incomplete: 0, priority: 0 },
  );

  const filterOpts = [
    { value: "all", label: "All classes & trainers" },
    ...LEVELS.map((l) => ({ value: `level:${l}`, label: `Level · ${l}` })),
    ...cohorts.map((c) => ({ value: `class:${c.id}`, label: `Class · ${c.name}` })),
    ...TRAINERS.filter((t) => classesOfTrainer(t.id).length || t.role).map((t) => ({ value: `trainer:${t.id}`, label: `Trainer · ${t.name}` })),
  ];
...
const catByKey = (key: string): Category | undefined => CATEGORIES.find((c) => c.key === key);

const ValueLine = ({ tone, catKey, note }: { tone: "up" | "down"; catKey: string; note: string }) => {
...
const CoachingCard = ({ rec, onOpenPlan, onOpenAdvocate, onLog }: {
  ...
  const coachName = rec.coachId ? (trainerById(rec.coachId)?.name || "—") : (rec.assignee || "—");
  return (
...
export const LogCoachingPanel = ({ rec, onClose }: { rec: EnrichedCoaching; onClose: () => void }) => {
  const store = useStore();
  const a = rec.advocate;
  const complete = rec.status === "complete";

  const [coachId, setCoachId] = useState<string>(rec.coachId || rec.classroomTrainer || TRAINERS[0].id);
  const [strengths, setStrengths] = useState<Sel[]>(rec.strengths || []);
  const [opportunities, setOpportunities] = useState<Sel[]>(rec.opportunities || []);
  ...
  const coachOpts = [
    ...TRAINERS.filter((t) => !t.role).map((t) => ({ value: t.id, label: t.name })),
    ...TRAINERS.filter((t) => t.role).map((t) => ({ value: t.id, label: `${t.name} · ${t.role}` })),
  ];

  const toggle = (l: Sel[], setList: (v: Sel[]) => void, key: string) => l.find((x) => x.key === key)
    ? setList(l.filter((x) => x.key !== key))
    : setList([...l, { key, note: "" }]);
  const setNote = (l: Sel[], setList: (v: Sel[]) => void, key: string, note: string) => setList(l.map((x) => x.key === key ? { ...x, note } : x));
      ...
      <div style={lDrawer} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 22, display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div>
            ...
            <Dropdown value={coachId} onChange={setCoachId} options={coachOpts} width={"100%" as any} align="left" />
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, padding: "16px 22px", borderTop: "1px solid var(--border-subtle)", background: "var(--canvas-default)" }}>
          <button style={lGhost} onClick={() => { store.saveCoachingNotes(rec.id, payload()); nestToast("Progress saved"); onClose(); }}>Save progress</button>
          {complete
            ? <button style={lGhost} onClick={() => { store.reopenCoaching(rec.id); nestToast("Coaching reopened"); onClose(); }}>Reopen</button>
            : <button style={{ ...lComplete, opacity: nothingSelected ? 0.5 : 1, pointerEvents: nothingSelected ? "none" : "auto" }} onClick={() => { store.completeCoaching(rec.id, payload()); nestToast(`Coaching marked complete for ${a.name.split(" ")[0]}`, "success"); onClose(); }}><NIcon.check s={16} /> Mark coaching complete</button>}
        </div>
      </div>
    </div>
  );
...
const cBadgeDone: React.CSSProperties = { display: "inline-flex", alignItems: "center", gap: 5, font: "var(--label-xs)", fontWeight: 700, color: "var(--text-success)", background: "var(--background-success-subtle)", borderRadius: 999, padding: "4px 10px", whiteSpace: "nowrap" };
const cBadgeOpen: React.CSSProperties = { display: "inline-flex", alignItems: "center", font: "var(--label-xs)", fontWeight: 700, color: "var(--text-warning)", background: "var(--background-warning-subtle)", borderRadius: 999, padding: "4px 10px" };
const cBadgePriority: React.CSSProperties = { display: "inline-flex", alignItems: "center", gap: 5, font: "var(--label-xs)", fontWeight: 700, color: "#fff", background: "var(--reference-pink-500)", borderRadius: 999, padding: "4px 10px", whiteSpace: "nowrap" };
const cGhostBtn: React.CSSProperties = { flex: 1, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6, height: 38, padding: "0 14px", borderRadius: 999, border: "1px solid var(--border-default)", background: "var(--canvas-default)", color: "var(--text-strong)", font: "var(--label-sm)", cursor: "pointer" };
const cPrimaryBtn: React.CSSProperties = { flex: 1, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6, height: 38, padding: "0 14px", borderRadius: 999, border: "none", background: "var(--background-primary)", color: "#fff", font: "var(--label-sm)", cursor: "pointer" };
const lScrim: React.CSSProperties = { position: "fixed", inset: 0, background: "var(--canvas-overlay, rgba(20,40,60,.55))", display: "flex", justifyContent: "flex-end", zIndex: 70, animation: "cdFade .18s ease" };
const lDrawer: React.CSSProperties = { width: 480, maxWidth: "94vw", height: "100%", background: "var(--canvas-muted)", display: "flex", flexDirection: "column", boxShadow: "-12px 0 40px rgba(13,55,94,.18)", animation: "cdSlide .26s cubic-bezier(.2,.7,.3,1)" };
const lClose: React.CSSProperties = { width: 36, height: 36, borderRadius: 999, border: "1px solid var(--border-default)", background: "var(--canvas-default)", color: "var(--icon-default)", display: "grid", placeItems: "center", cursor: "pointer" };
const lLabel: React.CSSProperties = { font: "var(--label-sm)", color: "var(--text-strong)", fontWeight: 600 };
const lGhost: React.CSSProperties = { height: 42, padding: "0 18px", borderRadius: 999, border: "1px solid var(--border-default)", background: "var(--canvas-default)", color: "var(--text-strong)", font: "var(--label-md)", cursor: "pointer" };
const lComplete: React.CSSProperties = { display: "inline-flex", alignItems: "center", gap: 7, height: 42, padding: "0 20px", borderRadius: 999, border: "none", background: "var(--background-success)", color: "#fff", font: "var(--label-md)", fontWeight: 700, cursor: "pointer" };
