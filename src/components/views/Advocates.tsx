...
// Advocates roster + individual drill-down (ported from app/advocates.jsx).
// Exports: AdvocatesView, AdvocateDetail, KV.
...
import { GradePill, ReadinessDonut, CatBars, momColor, scoreColor } from "@/components/charts";
import { BackBar } from "./Classes";

const _af1 = (v: number) => Number(v).toFixed(1);
const totalAdv = allAdvocates.length;

interface AdvocatesViewProps {
...
export const AdvocatesView = ({ onNav, onOpenAdvocate, onOpenPlan, search, onSearch }: AdvocatesViewProps) => {
  const [grade, setGrade] = useState("all");
  const [level, setLevel] = useState("all");

  const gradeOpts = [{ value: "all", label: "All grades" }, ...(["A", "B", "C", "D"] as const).map((g) => ({ value: g, label: `${g} · ${GRADES[g].label}` }))];
  const levelOpts = [{ value: "all", label: "All levels" }, ...LEVELS.map((l) => ({ value: l, label: l }))];
  ...
  return (
    <NestShell active="advocate" title="Advocates" subtitle={`${totalAdv} advocates across ${cohorts.length} classes`} onNav={onNav} searchValue={search} onSearch={onSearch}
      actions={<>
...
export const KV = ({ label, value, tone }: { label: ReactNode; value: ReactNode; tone?: "up" | "down" }) => (
...
const tdA2: React.CSSProperties = { padding: "12px 16px", verticalAlign: "middle" };
const aPlanBtn: React.CSSProperties = { display: "inline-flex", alignItems: "center", gap: 6, height: 32, padding: "0 13px", borderRadius: 999, border: "1px solid var(--brand-primary)", background: "var(--background-primary-subtle)", color: "var(--text-primary)", font: "var(--label-sm)", cursor: "pointer" };
const aDetailPrimary: React.CSSProperties = { display: "inline-flex", alignItems: "center", gap: 7, height: 40, padding: "0 18px", borderRadius: 999, border: "none", background: "var(--background-primary)", color: "#fff", font: "var(--label-md)", cursor: "pointer" };
