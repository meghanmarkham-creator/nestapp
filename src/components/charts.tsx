...
// The Nest — SVG visualization primitives (ported from app/charts.jsx).
// GradePill, ReadinessDonut, DistBar, Sparkline, TrendChart, CatBars, ContextBar,
// HeatCell, WeekDots + color scales (gradeColor, scoreColor, momColor).

import { GRADES, gradeOf } from "@/lib/nest-data";
import type { CatScore, ContextLevelAgg, Grade, GradeLetter } from "@/lib/types";

export const gradeColor = (letter: GradeLetter): Grade => GRADES[letter] || GRADES.D;

// score → continuous color across the A–D ramp (0–100)
...
export function momColor(v: number): string {
...
export const momFmt = (v: number) => Number(v).toFixed(1);

export const GradePill = ({ letter, size = 40, showLabel = false }: { letter: GradeLetter; size?: number; showLabel?: boolean }) => {
  const g = gradeColor(letter);
  return (
    <div style={{ display: "inline-flex", alignItems: "center", gap: 10 }}>
      <div style={{ width: size, height: size, borderRadius: 10, flexShrink: 0, background: g.subtle, color: g.text, display: "grid", placeItems: "center", font: "var(--heading-md)", fontWeight: 800, fontSize: size * 0.5, lineHeight: 1, border: `1.5px solid ${g.fill}` }}>{letter}</div>
      {showLabel && (
...
export const ReadinessDonut = ({ value, size = 168, stroke = 16, grade }: { value: number; size?: number; stroke?: number; grade?: Grade }) => {
  const g = grade || gradeOf(value);
  ...
  return (
    <div style={{ position: "relative", width: size, height: size }}>
      <svg width={size} height={size} style={{ transform: "rotate(-90deg)" }}>
        ...
        <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke={g.fill} strokeWidth={stroke} strokeLinecap="round" strokeDasharray={c} strokeDashoffset={c * (1 - pct)} />
      </svg>
...
export const DistBar = ({ dist, height = 12, showCounts = false, inBarCounts = false, gap = 2 }: { dist: Record<string, number>; total?: number; height?: number; showCounts?: boolean; inBarCounts?: boolean; gap?: number }) => {
  const order: GradeLetter[] = ["A", "B", "C", "D"];
  return (
        ...
        {order.map((l) => {
          ...
          return (
            <div key={l} title={`${l}: ${v}`} style={{ flex: v, background: gradeColor(l).fill, display: inBarCounts ? "flex" : "block", alignItems: "center", justifyContent: "center", gap: 4, minWidth: inBarCounts ? 22 : 0 }}>
              {inBarCounts && <span style={{ font: "var(--label-sm)", fontWeight: 800, color: light ? "var(--reference-slate-800)" : "rgba(255,255,255,.98)", lineHeight: 1, whiteSpace: "nowrap" }}>{v}</span>}
            </div>
          );
        })}
      </div>
...
export const Sparkline = ({ data, width = 96, height = 30, color = "var(--brand-primary)" }: { data: number[]; width?: number; height?: number; color?: string }) => {
...
export interface TrendSeries { data: number[]; color: string; dashed?: boolean; fill?: boolean }
export const TrendChart = ({ series, target, width = 520, height = 200, labels }: { series: TrendSeries[]; target?: number; width?: number; height?: number; labels?: string[] }) => {
  ...
  return (
    <svg width={width} height={height} style={{ display: "block", maxWidth: "100%" }}>
      ...
      {target != null && <line x1={pad.l} x2={width - pad.r} y1={y(target)} y2={y(target)} stroke="var(--reference-green-500)" strokeWidth="1.5" strokeDasharray="4 4" />}
      {series.map((s, si) => {
      ...
      {labels && labels.map((l, i) => (<text key={i} x={x(i)} y={height - 8} textAnchor="middle" style={{ font: "var(--body-regular-xs)", fill: "var(--text-weak)" }}>{l}</text>))}
    </svg>
  );
};

export const CatBars = ({ cats, width, max = 5, labelCol = 148 }: { cats: CatScore[]; width?: number | string; max?: number; labelCol?: number }) => {
  const isMom = max === 5;
  return (
...
export const ContextBar = ({ label, levels, height = 10 }: { label: string; levels: ContextLevelAgg[]; height?: number }) => (
...
export const HeatCell = ({ value, size = 30, radius = 6, label, title }: { value: number; size?: number; radius?: number; label?: string; title?: string }) => {
  const bg = scoreColor(value);
  return (
    <div title={title} style={{ width: size, height: size, borderRadius: radius, background: bg, display: "grid", placeItems: "center", cursor: "default", font: "var(--label-xs)", fontWeight: 700, color: value >= 78 ? "rgba(255,255,255,.96)" : "var(--reference-slate-800)" }}>{label}</div>
  );
};

export const WeekDots = ({ current, total }: { current: number; total: number }) => (
  <div style={{ display: "flex", gap: 4, alignItems: "center" }}>
    {Array.from({ length: total }).map((_, i) => (
      <span key={i} style={{ width: i < current ? 18 : 8, height: 8, borderRadius: 999, background: i < current ? "var(--brand-primary)" : "var(--reference-slate-200)", transition: "all .2s" }} />
    ))}
  </div>
);
