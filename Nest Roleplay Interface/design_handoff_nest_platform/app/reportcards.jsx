...
// Exports (to window): ReportCardsView, ArchiveView.

const _pf1 = (v) => Number(v).toFixed(1);
...
const leadFor = (a) => HANDOFF_LEADS[(a.name.charCodeAt(0) + a.composite) % HANDOFF_LEADS.length];

const ReportCardsView = ({ onNav, onOpenAdvocate, search, onSearch }) => {
  const N = window.NEST;
  ...
  const matches = q.trim()
    ? N.allAdvocates.filter(a => a.name.toLowerCase().includes(q.toLowerCase())).slice(0, 8)
    : [];
    ...
    <NestShell active="report" title="Report cards" subtitle="Search an advocate to open their full record" onNav={onNav} searchValue={search} onSearch={onSearch}
      ...
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px,1fr))", gap: 16 }}>
          {N.cohorts.filter(c => c.status === "Graduating").flatMap(c => c.advocates.slice(0, 6).map(a => ({ a, c }))).map(({ a, c }) => (
...
const Confetti = () => {
  const colors = ["var(--brand-tertiary)", "var(--reference-blue-400)", "var(--reference-green-400)", "var(--reference-pink-500)", "#fff"];
  const bits = React.useMemo(() => Array.from({ length: 28 }).map((_, i) => ({
    left: (i * 37) % 100, top: (i * 53) % 90, size: 6 + (i % 3) * 3, c: colors[i % colors.length], r: (i * 47) % 360,
  })), []);
  return (
...
const FullReportCard = ({ advId, onBack, onNav, onOpenAdvocate, search, onSearch }) => {
  const N = window.NEST;
  const store = window.useStore();
  const S = store.get();
  ...
  const myCalls = N.calls.filter(cl => cl.advId === advId);
  const myCoachings = window.buildCoachingList(N, S).filter(r => r.advId === advId);

  const metrics = [
    { label: "Average Roleplay MOM", value: _pf1(a.roleplayMom), unit: "/ 5", color: window.momColor(a.roleplayMom) },
    { label: "Average Production MOM", value: _pf1(a.productionMom), unit: "/ 5", color: window.momColor(a.productionMom) },
    { label: "Overall Assessment", value: a.assessment, unit: "/ 100", color: window.scoreColor(a.assessment) },
    { label: "Attendance", value: a.attendance, unit: "%", color: window.scoreColor(a.attendance) },
  ];
  ...
  const setMeeting = () => {
    ...
    window.nestToast(`Team Lead Handoff created for ${lead}`, "success");
  };
  const doGraduate = () => { store.graduate(advId, a.cohort); window.nestToast(`${a.name} graduated — moved to Archive`, "success"); };
  const doRelease = () => { store.release(advId); window.nestToast(`${a.name} released — priority coaching sent to Shelby Gary`, "success"); };
                ...
                <tbody>
                  {myCalls.map((cl, i) => (
                    <tr key={cl.id} style={{ borderTop: i ? "1px solid var(--border-subtle)" : "none" }}>
                      ...
                      <td style={{ padding: "10px 14px", font: "var(--body-regular-xs)", color: "var(--text-weak)", whiteSpace: "nowrap" }}>{N.fmtDur(cl.durationSec)}</td>
                      <td style={{ padding: "10px 14px", font: "var(--body-regular-xs)", color: "var(--text-weak)", whiteSpace: "nowrap" }}>{cl.date}</td>
                      <td style={{ padding: "10px 14px", textAlign: "right" }}>
                        {cl.mom != null ? <span style={{ font: "var(--body-strong-sm)", color: window.momColor(cl.mom) }}>{cl.mom.toFixed(1)}</span>
...
// ---- Admin-only Archive ----
const ArchiveView = ({ onNav, search, onSearch }) => {
  const N = window.NEST;
  const store = window.useStore();
  const S = store.get();
  const [q, setQ] = React.useState("");
  let rows = Object.entries(S.graduated).map(([advId, meta]) => ({ a: N.advocateById(advId), meta })).filter(r => r.a);
  if (q) rows = rows.filter(r => r.a.name.toLowerCase().includes(q.toLowerCase()));
  rows.sort((x, y) => new Date(y.meta.at) - new Date(x.meta.at));
...
const SectionH = ({ children }) => <div style={{ font: "var(--label-xs)", color: "var(--text-weak)", textTransform: "uppercase", letterSpacing: ".05em", fontWeight: 600, marginBottom: 12 }}>{children}</div>;

const rGhost = { display: "inline-flex", alignItems: "center", gap: 7, height: 40, padding: "0 16px", borderRadius: 999, border: "1px solid var(--border-default)", background: "var(--canvas-default)", color: "var(--text-strong)", font: "var(--label-md)", cursor: "pointer" };
const rCard = { width: "100%", maxWidth: 880, background: "var(--canvas-default)", border: "1px solid var(--border-subtle)", borderRadius: "var(--border-radius-xl)", padding: 34 };
const rMeeting = { display: "inline-flex", alignItems: "center", gap: 7, height: 40, padding: "0 18px", borderRadius: 999, border: "none", background: "var(--background-warning)", color: "var(--brand-secondary)", font: "var(--label-md)", fontWeight: 700, cursor: "pointer" };
const rGraduate = { display: "inline-flex", alignItems: "center", gap: 8, height: 46, padding: "0 28px", borderRadius: 999, border: "none", background: "var(--background-success)", color: "#fff", font: "var(--label-lg)", fontWeight: 700, cursor: "pointer", boxShadow: "0 6px 18px rgba(31,138,91,.3)" };
const rRelease = { display: "inline-flex", alignItems: "center", gap: 8, height: 46, padding: "0 24px", borderRadius: 999, border: "1.5px solid var(--reference-yellow-500)", background: "var(--background-warning-subtle)", color: "var(--brand-secondary)", font: "var(--label-lg)", fontWeight: 700, cursor: "pointer" };

Object.assign(window, { ReportCardsView, ArchiveView, FullReportCard });
