...
// advocates + trainers into classes. Exports (to window): ClassAssignmentsView.

const ClassAssignmentsView = ({ onNav, search, onSearch }) => {
  const N = window.NEST;
  ...
  const [dragAdv, setDragAdv] = React.useState(null);
  const [overClass, setOverClass] = React.useState(null);
  ...
  const orgPool = N.allAdvocates.filter(a => !assignedIds.has(a.id) && !store.isGraduated(a.id));
  const pool = [...manual.filter(m => !assignedIds.has(m.id)), ...orgPool]
    .filter(a => !q || a.name.toLowerCase().includes(q.toLowerCase()) || (a.email || "").toLowerCase().includes(q.toLowerCase()));

  const advName = (id) => (N.advocateById(id) || S.manualAdvocates.find(m => m.id === id) || {}).name || id;
  const advObj = (id) => N.advocateById(id) || S.manualAdvocates.find(m => m.id === id);

  const drop = (classId) => {
    if (dragAdv) { store.assignAdvocate(dragAdv, classId); window.nestToast(`${advName(dragAdv).split(" ")[0]} added to class`, "success"); }
  ...
  return (
    <NestShell active="assign" title="Class Assignments" subtitle="Build cohorts from the org roster — drag advocates and a trainer into a class" onNav={onNav} searchValue={search} onSearch={onSearch}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <Panel title="Nest trainers" subtitle="Drag a trainer onto a class to assign them as coach" pad={16}>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
              {N.TRAINERS.filter(t => !t.role).map(t => (
                <div key={t.id} draggable onDragStart={(e) => { e.dataTransfer.setData("trainer", t.id); setDragAdv(null); }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 16 }}>
            {S.customClasses.map(cls => {
              const trainer = cls.trainerId ? N.trainerById(cls.trainerId) : null;
              const isOver = overClass === cls.id;
              return (
                <div key={cls.id}
                  onDragOver={(e) => { e.preventDefault(); setOverClass(cls.id); }}
                  onDragLeave={() => setOverClass(o => o === cls.id ? null : o)}
                  onDrop={(e) => { const tid = e.dataTransfer.getData("trainer"); if (tid) { store.assignTrainer(cls.id, tid); window.nestToast(`${N.trainerById(tid).name} assigned to ${cls.name}`, "success"); setOverClass(null); } else drop(cls.id); }}
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10 }}>
                    ...
                    <button style={aTrash} onClick={() => { store.deleteClass(cls.id); window.nestToast("Class deleted"); }} title="Delete class"><NIcon.x s={15}/></button>
                  </div>
                  ...
                  <div>
                    ...
                    {trainer ? (
                      <div style={{ marginTop: 6 }}><TrainerChip trainer={trainer} compact onRemove={() => store.assignTrainer(cls.id, null)}/></div>
                    ) : (
      ...
      {creating && <CreateClassModal onClose={() => setCreating(false)} onCreate={(payload) => { store.createClass(payload); window.nestToast(`Class “${payload.name}” created`, "success"); setCreating(false); }}/>}
      {adding && <AddStudentModal onClose={() => setAdding(false)} onAdd={(payload) => { store.addManualAdvocate(payload); window.nestToast(`${payload.name} added to the pool`, "success"); setAdding(false); }}/>}
    </NestShell>
  );
};
...
const CreateClassModal = ({ onClose, onCreate }) => {
  const N = window.NEST;
  const [level, setLevel] = React.useState("New Hire");
  const [date, setDate] = React.useState("2026-07-06");
...
const aGhost = { display: "inline-flex", alignItems: "center", gap: 7, height: 40, padding: "0 16px", borderRadius: 999, border: "1px solid var(--border-default)", background: "var(--canvas-default)", color: "var(--text-strong)", font: "var(--label-md)", cursor: "pointer" };
const aPrimary = { display: "inline-flex", alignItems: "center", gap: 7, height: 40, padding: "0 18px", borderRadius: 999, border: "none", background: "var(--background-primary)", color: "#fff", font: "var(--label-md)", cursor: "pointer" };
const aTrash = { width: 30, height: 30, borderRadius: 8, border: "1px solid var(--border-subtle)", background: "var(--canvas-default)", color: "var(--icon-subtle)", display: "grid", placeItems: "center", cursor: "pointer", flexShrink: 0 };
const aSlotLabel = { font: "var(--label-xs)", color: "var(--text-weak)", textTransform: "uppercase", letterSpacing: ".05em", fontWeight: 600 };
const aInput = { width: "100%", height: 42, padding: "0 14px", borderRadius: 8, border: "1px solid var(--border-default)", font: "var(--body-regular-md)", outline: "none", boxSizing: "border-box" };
const eScrim2 = { position: "fixed", inset: 0, background: "var(--canvas-overlay, rgba(20,40,60,.55))", display: "grid", placeItems: "center", zIndex: 70, padding: 20 };
const aModal = { background: "var(--canvas-default)", borderRadius: "var(--border-radius-xl)", padding: 26, width: "100%", maxWidth: 460, boxShadow: "0 24px 60px rgba(13,55,94,.3)" };

window.ClassAssignmentsView = ClassAssignmentsView;
