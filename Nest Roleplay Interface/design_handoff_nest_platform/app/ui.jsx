// Nest — shared UI atoms reused across views.
// Exports (to window): Dropdown, Avatar, StatusBadge, LevelTag, initials,
//   gradeStatusStyle, EmptyState, Segmented.

const initials = (name) => name.split(" ").map(x => x[0]).join("").slice(0, 2).toUpperCase();

// click-to-open dropdown (used for the cohort selector)
const Dropdown = ({ value, options, onChange, placeholder = "Select", icon, width = 220, align = "left" }) => {
  ...
  return (
    <div ref={ref} style={{ position: "relative", width }}>
      <button onClick={() => setOpen(o => !o)} style={{
        display: "flex", alignItems: "center", gap: 8, width: "100%", height: 40, padding: "0 12px 0 14px",
        borderRadius: 999, border: `1px solid ${open ? "var(--brand-primary)" : "var(--border-default)"}`,
        background: "var(--canvas-default)", color: "var(--text-strong)", font: "var(--label-md)", cursor: "pointer",
      }}>
      ...
      {open && (
        <div style={{
          position: "absolute", top: 46, [align]: 0, minWidth: "100%", zIndex: 30,
          background: "var(--canvas-default)", border: "1px solid var(--border-subtle)",
          borderRadius: "var(--border-radius-md)", boxShadow: "0 12px 32px rgba(13,55,94,.16)",
          ...
          {options.map(o => {
            ...
            return (
              <div key={o.value} onClick={() => { onChange(o.value); setOpen(false); }} style={{
                ...
                background: sel ? "var(--background-primary-subtle)" : "transparent",
                color: sel ? "var(--text-primary)" : "var(--text-default)", font: sel ? "var(--body-strong-sm)" : "var(--body-regular-sm)",
              }}
...
// segmented control (level filter)
    ...
    {options.map(o => {
      ...
      return (
        <button key={o.value} onClick={() => onChange(o.value)} style={{
          font: "var(--label-md)", padding: "7px 15px", borderRadius: 999, cursor: "pointer", whiteSpace: "nowrap",
          border: on ? "1px solid var(--brand-primary)" : "1px solid var(--border-default)",
          background: on ? "var(--background-primary-subtle)" : "var(--canvas-default)",
          color: on ? "var(--text-primary)" : "var(--text-default)",
        }}>{o.label}{o.count != null && <span style={{ marginLeft: 7, opacity: .7 }}>{o.count}</span>}</button>
      );
    })}
  </div>
);

const Avatar = ({ name, size = 36, grade }) => {
  const g = grade ? window.NEST.GRADES[grade] : null;
  return (
    <div style={{
      width: size, height: size, borderRadius: 999, flexShrink: 0,
      background: g ? g.subtle : "var(--reference-slate-100)",
      color: g ? g.text : "var(--text-strong)",
      border: g ? `1.5px solid ${g.fill}` : "none",
      display: "grid", placeItems: "center", font: "var(--label-xs)", fontWeight: 700,
      fontSize: size * 0.34,
    }}>{initials(name)}</div>
  );
};
...
// Trainer identity — deliberately distinct from advocates (navy fill, white text)
const TrainerAvatar = ({ name, size = 36 }) => (
  <div style={{
    width: size, height: size, borderRadius: 10, flexShrink: 0,
    background: "var(--brand-secondary)", color: "#fff",
    display: "grid", placeItems: "center", font: "var(--label-xs)", fontWeight: 800, fontSize: size * 0.34,
    boxShadow: "inset 0 0 0 1.5px var(--brand-tertiary)",
  }}>{initials(name)}</div>
);

const TrainerChip = ({ trainer, onRemove, draggable, onDragStart, compact }) => (
  <div draggable={draggable} onDragStart={onDragStart} style={{
    display: "inline-flex", alignItems: "center", gap: 9, padding: compact ? "5px 10px 5px 6px" : "7px 12px 7px 7px",
    borderRadius: 999, background: "var(--brand-secondary)", color: "#fff",
    boxShadow: "inset 0 0 0 1.5px var(--brand-tertiary)", cursor: draggable ? "grab" : "default", whiteSpace: "nowrap",
  }}>
    ...
    {onRemove && <span onClick={(e) => { e.stopPropagation(); onRemove(); }} style={{ display: "inline-flex", cursor: "pointer", opacity: .7 }}><NIcon.x s={14}/></span>}
  </div>
);

const StatusDot = ({ statusKey }) => {
  const s = (window.NEST.TRAINER_STATUSES.find(x => x.key === statusKey)) || window.NEST.TRAINER_STATUSES[0];
  return (
...
// Lightweight toast (bottom-center). Call window.nestToast(msg).
const ToastHost = () => {
  const [toasts, setToasts] = React.useState([]);
  React.useEffect(() => {
    window.nestToast = (msg, tone = "default") => {
    ...
    <div style={{ position: "fixed", bottom: 24, left: "50%", transform: "translateX(-50%)", display: "flex", flexDirection: "column", gap: 8, zIndex: 90, alignItems: "center" }}>
      {toasts.map(t => (
        <div key={t.id} style={{
          ...
          background: t.tone === "success" ? "var(--background-success)" : "var(--brand-secondary)",
          color: "#fff", font: "var(--body-strong-sm)", boxShadow: "0 8px 24px rgba(13,55,94,.28)", animation: "cdFade .18s ease",
        }}>
...
Object.assign(window, { Dropdown, Segmented, Avatar, StatusBadge, LevelTag, initials, EmptyState, LEVEL_COLOR, TrainerAvatar, TrainerChip, StatusDot, ToastHost });
