...
// ACS transcript preview. Exports (to window): AIEfficiencyView.
...
const AIEfficiencyView = ({ onNav, onOpenAdvocate, search, onSearch }) => {
  const N = window.NEST;
  const store = window.useStore();
  const S = store.get();
  const [acsCall, setAcsCall] = React.useState(null);
  const [omitFor, setOmitFor] = React.useState(null);
  const [tab, setTab] = React.useState("cutoff");

  const omittedIds = Object.keys(S.omits);
  const cutoff = N.cutoffCalls;
  const failed = N.failedCalls;
  const pctOmitted = Math.round((omittedIds.length / N.calls.length) * 1000) / 10;

  const list = tab === "cutoff" ? cutoff : tab === "failed" ? failed : N.calls.filter(c => S.omits[c.id]);
  ...
  return (
    <NestShell active="aieff" title="AI Efficiency" subtitle="AI scoring health across roleplay and production calls" onNav={onNav} searchValue={search} onSearch={onSearch}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px,1fr))", gap: 16 }}>
        ...
        <StatTile label="% omitted from scoring" value={`${pctOmitted}%`} sub={`${omittedIds.length} of ${N.calls.length}`} accent="var(--reference-orange-500)"/>
        <StatTile label="Avg call length" value={N.fmtDur(N.avgCallLen)} sub="min:sec" accent="var(--brand-primary)"/>
        <StatTile label="Calls scored" value={N.scoredCount} sub={`of ${N.calls.length}`} accent="var(--reference-green-500)"/>
                  ...
                  <tr key={call.id} className="nest-row" style={{ borderTop: "1px solid var(--border-subtle)", opacity: omitted ? 0.6 : 1 }}>
                    ...
                    <td style={tdE}>
                      <span style={{ display: "inline-flex", alignItems: "center", gap: 6, font: "var(--body-strong-sm)", color: call.cutoff ? "var(--text-critical)" : "var(--text-default)" }}>
                        <NIcon.clock s={14}/>{N.fmtDur(call.durationSec)}
                      </span>
                    </td>
                    ...
                    <td style={{ ...tdE, textAlign: "center" }}>
                      {call.mom != null ? <span style={{ font: "var(--body-strong-sm)", color: window.momColor(call.mom) }}>{call.mom.toFixed(1)}</span>
      ...
      {omitFor && <OmitModal call={omitFor} onClose={() => setOmitFor(null)} onConfirm={(reason) => { store.omitCall(omitFor.id, reason); const a = N.advocateById(omitFor.advId); window.nestToast(`Call omitted from ${a.name.split(" ")[0]}'s scoring`, "success"); setOmitFor(null); }}/>}
    </NestShell>
  );
};

// ---- ACS transcript / MOM preview modal ----
const ACSModal = ({ call, onClose }) => {
  const N = window.NEST;
  const a = N.advocateById(call.advId);
  const lines = N.transcriptFor(call);
  return (
      ...
      <div style={eModal} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 22px", borderBottom: "1px solid var(--border-subtle)" }}>
          <div>
            ...
            <div style={{ font: "var(--body-regular-sm)", color: "var(--text-weak)" }}>{a.name} · {call.scenario} · {N.fmtDur(call.durationSec)} · {call.date}</div>
          </div>
            ...
            {call.mom != null ? (
              <div style={{ marginTop: 12 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
                  <span style={{ font: "var(--heading-md)", fontWeight: 800, color: window.momColor(call.mom) }}>{call.mom.toFixed(1)}</span>
        ...
        <div style={{ padding: "14px 22px", borderTop: "1px solid var(--border-subtle)", display: "flex", justifyContent: "flex-end", gap: 10 }}>
          ...
          <a style={eGhost} onClick={() => window.nestToast("Opening full record in ACS…")}><NIcon.ext s={15}/> Open in ACS</a>
        </div>
      </div>
    </div>
  );
...
const OmitModal = ({ call, onClose, onConfirm }) => {
  const N = window.NEST;
  const a = N.advocateById(call.advId);
  const [reason, setReason] = React.useState(OMIT_REASONS[0]);
...
const tdE = { padding: "12px 16px", verticalAlign: "middle" };
const eLink = { display: "inline-flex", alignItems: "center", gap: 5, font: "var(--body-link-sm)", color: "var(--text-primary)", cursor: "pointer", whiteSpace: "nowrap" };
const eOmitBtn = { display: "inline-flex", alignItems: "center", gap: 6, height: 32, padding: "0 14px", borderRadius: 999, border: "1px solid var(--reference-yellow-500)", background: "var(--background-warning-subtle)", color: "var(--text-warning)", font: "var(--label-sm)", cursor: "pointer" };
const eRestoreBtn = { display: "inline-flex", alignItems: "center", gap: 6, height: 32, padding: "0 14px", borderRadius: 999, border: "1px solid var(--border-default)", background: "var(--canvas-default)", color: "var(--text-strong)", font: "var(--label-sm)", cursor: "pointer" };
const eScrim = { position: "fixed", inset: 0, background: "var(--canvas-overlay, rgba(20,40,60,.55))", display: "grid", placeItems: "center", zIndex: 70, padding: 20 };
const eModal = { background: "var(--canvas-default)", borderRadius: "var(--border-radius-xl)", width: "100%", maxWidth: 620, maxHeight: "86vh", display: "flex", flexDirection: "column", boxShadow: "0 24px 60px rgba(13,55,94,.3)", overflow: "hidden" };
const eClose = { width: 36, height: 36, borderRadius: 999, border: "1px solid var(--border-default)", background: "var(--canvas-default)", color: "var(--icon-default)", display: "grid", placeItems: "center", cursor: "pointer", flexShrink: 0 };
const eSectionLabel = { font: "var(--label-xs)", color: "var(--text-weak)", textTransform: "uppercase", letterSpacing: ".05em", fontWeight: 600 };
const eGhost = { display: "inline-flex", alignItems: "center", gap: 6, height: 40, padding: "0 18px", borderRadius: 999, border: "1px solid var(--border-default)", background: "var(--canvas-default)", color: "var(--text-strong)", font: "var(--label-md)", cursor: "pointer" };
const eOmitConfirm = { height: 40, padding: "0 20px", borderRadius: 999, border: "none", background: "var(--background-warning)", color: "var(--brand-secondary)", font: "var(--label-md)", fontWeight: 700, cursor: "pointer" };

window.AIEfficiencyView = AIEfficiencyView;
