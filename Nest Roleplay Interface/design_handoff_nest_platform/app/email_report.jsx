// Stakeholder email report — a branded, copy-ready email template built from
// the current Readiness snapshot. Exports (to window): StakeholderEmailModal.

const StakeholderEmailModal = ({ onClose, scope }) => {
  const N = window.NEST;
  ...
  const HEX = { A: "#1f8a5b", B: "#228be6", C: "#fab005", D: "#e03131" };
  const GRADE_LABEL = { A: "Ready", B: "On track", C: "Needs support", D: "At risk" };
  const gh = HEX[g.letter];

  const pct = (n) => Math.round((n / scope.total) * 100);
  const distSegs = ["A", "B", "C", "D"].filter(l => scope.dist[l] > 0);
  ...
  const copyEmail = async () => {
    ...
    try {
      ...
      window.nestToast("Email copied — paste into Outlook or Gmail", "success");
    } catch (e) {
      try { await navigator.clipboard.writeText(plain); window.nestToast("Copied as text", "success"); }
      catch (e2) { window.nestToast("Copy failed — select and copy manually"); }
    }
  ...
  const downloadHtml = () => {
    const doc = `<!doctype html><html><body style="margin:0">${previewRef.current.innerHTML}</body></html>`;
    ...
    window.nestToast("Downloaded email HTML");
  };

  // email-safe inline styles
  const cell = { fontFamily: "Inter, Arial, sans-serif" };
...
const emScrim = { position: "fixed", inset: 0, background: "var(--canvas-overlay, rgba(20,40,60,.55))", display: "grid", placeItems: "center", zIndex: 80, padding: 20 };
const emModal = { background: "var(--canvas-default)", borderRadius: "var(--border-radius-xl)", width: "100%", maxWidth: 760, maxHeight: "92vh", display: "flex", flexDirection: "column", boxShadow: "0 24px 60px rgba(13,55,94,.3)", overflow: "hidden" };
const emClose = { width: 36, height: 36, borderRadius: 999, border: "1px solid var(--border-default)", background: "var(--canvas-default)", color: "var(--icon-default)", display: "grid", placeItems: "center", cursor: "pointer", flexShrink: 0 };
const emGhost = { display: "inline-flex", alignItems: "center", gap: 7, height: 40, padding: "0 16px", borderRadius: 999, border: "1px solid var(--border-default)", background: "var(--canvas-default)", color: "var(--text-strong)", font: "var(--label-md)", cursor: "pointer" };
const emPrimary = { display: "inline-flex", alignItems: "center", gap: 7, height: 40, padding: "0 20px", borderRadius: 999, border: "none", background: "var(--background-primary)", color: "#fff", font: "var(--label-md)", cursor: "pointer" };

window.StakeholderEmailModal = StakeholderEmailModal;
