...
// analysis, plus a self-service status selector. Exports: TrainersView.

const TrainersView = ({ onNav, onOpenClass, onOpenAdvocate, search, onSearch }) => {
  const N = window.NEST;
  const store = window.useStore();
  const S = store.get();
  const roster = N.TRAINERS.filter(t => N.classesOfTrainer(t.id).length > 0);
  const [selId, setSelId] = React.useState(roster[0]?.id);
  const trainer = N.trainerById(selId);
  const insight = selId ? N.trainerInsights(selId) : null;
  const classes = selId ? N.classesOfTrainer(selId) : [];
  const ranking = N.trainerRanking();
  const rank = ranking.findIndex(r => r.trainer.id === selId) + 1;
  const statusKey = S.trainerStatus[selId] || N.defaultTrainerStatus(selId);
  ...
  return (
    <NestShell active="trainers" title="Trainers" subtitle={`${roster.length} Nest trainers · class performance and trends`} onNav={onNav} searchValue={search} onSearch={onSearch}>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
        {roster.map(t => {
          const on = t.id === selId;
          const ti = N.trainerInsights(t.id);
          return (
          ...
          <Panel title="AI trend analysis" subtitle={`Patterns across ${trainer.name.split(" ")[0]}'s classes`}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px,1fr))", gap: 14 }}>
              ...
              <InsightCard
                ...
                body={`Classes average ${insight.avg} readiness — ${Math.abs(insight.gap)} pts ${insight.gap >= 0 ? "above" : "below"} the program average of ${N.programAvg}. ${trainer.name.split(" ")[0]} ranks #${rank} of ${ranking.length} trainers.`}/>
              <InsightCard
...
const InsightCard = ({ icon, tone, title, body }) => {
  const map = {
  ...
  return (
    <div style={{ background: "var(--canvas-default)", border: "1px solid var(--border-subtle)", borderRadius: "var(--border-radius-lg)", padding: 16, display: "flex", flexDirection: "column", gap: 8 }}>
      <div style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
        <span style={{ width: 30, height: 30, borderRadius: 8, background: map[0], color: map[1], display: "grid", placeItems: "center", flexShrink: 0 }}>{icon}</span>
...
const tdT = { padding: "12px 16px", verticalAlign: "middle" };
const trTag = { display: "inline-flex", alignItems: "center", gap: 5, font: "var(--label-xs)", fontWeight: 600, color: "var(--text-primary)", background: "var(--background-primary-subtle)", borderRadius: 999, padding: "4px 10px" };

window.TrainersView = TrainersView;
